...
function doPost(e) {
  ...
  try {
    ...
    switch (data.formType) {
      ...
      // --- NEW CASES FOR IMAGE TAGGING ---
      case 'uploadTaggedImages': 
        response = handleUploadTaggedImages(data);
        break;

      case 'saveImageLinks':
        response = handleSaveImageLinks(data);
        break;
  ...
  // This part now handles both 'no-cors' and standard requests
  if (e.postData.type === 'application/json') {
      return ContentService
        .createTextOutput(JSON.stringify(response))
        .setMimeType(ContentService.MimeType.JSON);
  } else {
      // This part is for the 'no-cors' submissions that expect a plain text response
 ...
 */
function handleUploadTaggedImages(data) {
  try {
    const folderId = "14huA4oOrebxd3L0Ubgv9N9PGcfADP_-4"; // <-- IMPORTANT: PASTE YOUR FOLDER ID HERE
    const folder = DriveApp.getFolderById(folderId);

    // Decode the Base64 string into a blob (binary data)
    const base64Data = data.imageData.split(',')[1];
    const decoded = Utilities.base64Decode(base64Data);
    const blob = Utilities.newBlob(decoded, data.mimeType, data.imageName);
 ...
 */
function handleSaveImageLinks(data) {
  try {
    const sheetName = 'QBoulderPhotos';
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(sheetName);
    if (!sheet) throw new Error(`Sheet "${sheetName}" not found.`);

    const rowsToAdd = data.images.map(image => {
      return [
        data.quali,
        image.name,
        image.link,
        JSON.stringify(image.tags) // Store the tag data as a JSON string
      ];
    });
    ...
    return { result: 'success', message: 'All image links saved successfully.' };
  } catch (error) {
